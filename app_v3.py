...
class JobSeekerProfile:
    ...
    # Learning preference is made up of three sub-components. learning_styles
    # (above) already covers one of them; these two cover the others.
    lifelong_learning_mindset: Optional[str] = None
    goal_orientation: Optional[str] = None
    # Empowerment is NOT a single combined score — it's kept as one score per
    # dimension (see EMPOWERMENT_DIMENSIONS below), since collapsing it into
    # one number would lose exactly the distinctions caseworkers need to see.
    empowerment: dict = field(
...
# The nine dimensions empowerment is measured across. These are display-only
# for now (see caseworker profile view) — none of them feed check_feasibility
# or rank_courses, since courses have no per-dimension empowerment attribute
# to match against.
...
# Friendlier labels for the caseworker-facing display. Update this if the
# intended meaning of any dimension (especially the abbreviated ones) turns
# out to differ from what's assumed here.
EMPOWERMENT_DIMENSION_LABELS = {
    ...
    "res_stress": "Resilience to stress",
}
def extract_empowerment_scores(user):
    """
    Pulls each empowerment dimension off the user row if present. Missing
    dimensions are recorded as None rather than skipped, so the caseworker
    view can show "not available" instead of silently omitting a dimension.
    """
    scores = {}
    for dimension in EMPOWERMENT_DIMENSIONS:
        value = user.get(dimension, None)
...
def extract_jobseeker_profile(user):
    ...
    profile = JobSeekerProfile(
        ...
        lifelong_learning_mindset=(
            user["lifelong_learning_mindset"]
            if "lifelong_learning_mindset" in user
            and pd.notna(user["lifelong_learning_mindset"])
            else None
        ),
        goal_orientation=(
            user["goal_orientation"]
            if "goal_orientation" in user
            and pd.notna(user["goal_orientation"])
            else None
        ),
        empowerment=extract_empowerment_scores(user),
